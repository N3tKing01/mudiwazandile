...
async function kickCommand(sock, chatId, senderId, mentionedJids, message) {
    ...
    if (!isOwner) {
        ...
        if (!isBotAdmin) {
            await sock.sendMessage(chatId, { text: 'Sicela wenze the bot an kuphela first.' }, { quoted: createFakeContact(message) });
            return;
        }

        if (!isSenderAdmin) {
            await sock.sendMessage(chatId, { text: 'Abaphathi beqembu kuphela can use the kick command.' }, { quoted: createFakeContact(message) });
            return;
        }
    }
