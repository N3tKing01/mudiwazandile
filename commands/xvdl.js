...
async function xvdlCommand(sock, chatId, message) {
    try {
        ...
        if (!apiData.status || !apiData.result || !apiData.result.download_url) {
            throw new Error("API yehlulekile ukulanda video!");
        }
        ...
        if (error.message.includes("timeout")) {
            ...
            errorMessage = "🔧 API error! Ukuzama futhi in a few moments.";
        } else if (error.message.includes("empty file")) {
            errorMessage = "📭 Download yehlulekile! Video might not be available.";
        }
