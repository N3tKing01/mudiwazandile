...
async function githubCommand(sock, chatId, message) {
    try {
        ...
        const res = await fetch('https://api.github.com/repos/JoshKingmd/CRAZY-X');
        if (!res.ok) throw new Error('Error fetching repository data');
        const json = await res.json();

        let txt = `🔸  \`𝚂𝙸𝙻𝙴𝙽𝚃_𝙻𝙳𝚃 𝚁𝙴𝙿𝙾 𝙸𝙽𝙵𝙾🔸\` \n\n`;
        txt += `🔹  *Name* : ${json.name}\n`;
        ...
        txt += `Hey👋 @${senderPhone} \n Thank you for choosing Crazy x Bot, fork and Star the repository_`;
