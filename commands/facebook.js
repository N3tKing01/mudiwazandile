    ...
    try {
        ...
        if (!text) {
            return await sock.sendMessage(chatId, { 
                text: "Sicela unikeze isixhumanisi se-Facebook sevidiyo."
            }, { quoted: fakeQuoted });
        }
        ...
        if (!isValidUrl) {
            return await sock.sendMessage(chatId, { 
                text: "That is not a valid Facebook link. Sicela unikeze isixhumanisi sevidiyo se-Facebook esisebenzayo."
            }, { quoted: fakeQuoted });
        }
        ...
        try {
            ...
            if (data && data.status && data.result && data.result.media.sd && data.result.media.hd) {
                ...
                return await sock.sendMessage(chatId, {
                    text: "Ndatadza kuita fetch video. Chikumbiro check the link or zama muchikamu chinotevera."
                }, { quoted: fakeQuoted });
            }
            ...
            await sock.sendMessage(chatId, {
                text: "Ndatadza kudownloader dis Facebook video. Chikumbiro zama muchikamu chinotevera."
            }, { quoted: fakeQuoted });
        }
    } catch (error) {
        console.error('Error in facebookCommand:', error.message || error);
        await sock.sendMessage(chatId, {
            text: "An unexpected error occurred. Chikumbiro zama futi."
